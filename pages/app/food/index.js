...
export default function Food(){
        ...
        .then(async (res)=>{
            await res.json().then(async (data)=>{
                ...
                if (data.length > 0) {
                    ...
                    localStorage.setItem('calories_consumption',calories)
                } else {
                    setCalories(0)
	...
	return levelPack.food_categories ? (
		<section>
			...
			<div className={`bg-form ${nutritionLogs > 0 || nutritions > 0 ? 'h-full' : 'h-screen'} base-background flex items-center flex-col py-5`}>
